...
Masses were converted from simulation units to physical $M_\odot$ using the $10^{10}/h$ scaling given in the IllustrisTNG documentation \citep{IllustrisTNG_Documentation}. To enable log--log visualization, objects with non-finite or non-positive halo mass or black hole mass were excluded. Because the catalogs contain millions of entries, relations were visualized with a log-scaled \texttt{hexbin} density plot to avoid overplotting while retaining information about the number density in parameter space.
...
At low halo masses, the SubFind relation exhibits noticeably larger scatter than the FoF relation. This is likely a consequence of the black hole seeding prescription in the simulation: black holes are introduced only once halos exceed a threshold mass and begin with a fixed seed mass \citep{Weinberger2017TNG_BHFB}. Many low-mass subhalos therefore host either no black hole or a newly seeded one, producing substantial dispersion.
...
Overall, the SubFind relation primarily traces black hole--galaxy co-evolution, while FoF reflects the connection between black holes and the assembly history of their host dark matter halos.
...
First, the black hole seeding prescription introduces artificial floors in many of the relations \citep{Weinberger2017TNG_BHFB}. Black holes are inserted with a fixed seed mass once halos exceed a threshold mass, producing a population of galaxies with similar black hole masses at the low-mass end. This results in horizontal features in log--log space and increased scatter that are numerical artifacts rather than physical predictions. Any quantitative interpretation of the low-mass regime must therefore be treated with caution.
...
If this work were to be published, additional robustness tests would be necessary, as emphasized in the IllustrisTNG data release documentation and methodology papers \citep{Nelson2019TNG_PublicData}. These would include resolution convergence checks, sensitivity tests to the seeding prescription, and comparisons across different halo mass definitions.
...
Overall, the simulation reproduces a realistic $M_{\rm BH}$--$\sigma$ scaling consistent with observational determinations \citep{Tremaine2002Msigma,KormendyHo2013}, indicating that the
...
\textcolor{blue}{\textbf{Prompt:} Now examine the actual black hole accretion rates as listed in the TNG catalogs.}
...
Black hole growth in TNG is strongly self-regulated by AGN feedback \citep{Weinberger2017TNG_BHFB,DiMatteo2005SelfReg,HeckmanBest2014}.
